...
void setup()
{
  ...
  Serial.println("Start, Arduino!"); // 시리얼 포트 TX로 Start, Arduino! 출력
  Wire.begin(8);                     // I2C BUS 8번으로 지정, 슬레이브로 등록
  Wire.onReceive(receiveEvent);      // I2C 수신시 호출될 함수 등록
  Wire.onRequest(requestEvent);      // I2C 송신 요청시 호출될 함수 등록
  pinMode(ledRED, OUTPUT);           // LED핀 출력모드, RED는 I2C 통신시 사용
  pinMode(ledGREEN, OUTPUT);         // LED핀 출력모드, GREEN은 센서 수집시 사용
  pinMode(ultrasonic_Trig, OUTPUT); // 초음파센서 출력핀 설정
  pinMode(ultrasonic_Echo, INPUT);  // 초음파센서 입력핀 설정
  pinMode(gasSensor, INPUT);        // 가스센서 입력핀 설정
  initLCD();                        // LCD 초기화
...
PressureSensor *Pressure_Check() // 대기압 측정
{
  ...
  ps->Faltitude = bmp.readAltitude(101600); // 101600 : 인천 송도 해면기압, 101325 : 표준기압, 각 지역별 기압은 기상청 참고
...
{
  if (sensorType == temper_Hum)         // 온습도 센서
  {
    // Serial 출력
  ...
  else if (sensorType == ultrasonic_Trig)   // 초음파 센서
  {
    Serial.print("LCD ");
  ...
  else if (sensorType == illuminSensor)     // 조도 센서
  {
  ...
  else if (sensorType == gasSensor)         // 가스 센서
  {
  ...
  else if (sensorType == dustSensor)        // 미세먼지 센서
  {
  ...
  else if (sensorType == pressureSensor_SCL || pressureSensor_SDA)      // 대기압 센서
  {
    Serial.print("LCD ");
...
void requestEvent()
{
  ...
  else if(buf == "Update Status")           // 센서데이터 업데이트 유무 확인
  {
    ...
    if(update == true)                      // 센서 데이터 업데이트 완료시 마스터에게 TR 전송
    {
      Wire.write("TR");
      Serial.println("Transmission Update Status : TR");
    }
    else if(update == false)                // 센서 데이터 업데이트 미완료시 마스터에게 FS 전송
    {
      Wire.write("FS");
